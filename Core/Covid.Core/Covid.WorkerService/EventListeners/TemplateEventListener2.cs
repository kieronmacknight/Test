...
namespace Covid.WorkerService.EventListeners
{
    sealed class TemplateEventListener2
    {
        ...
        public TemplateEventListener2(Api api)
        {
            if (api == null)
        ...
        public async Task Run(CancellationToken cancellationToken)
        {
            try
            {
                ...
                while (!cancellationToken.IsCancellationRequested && cnt < 500)
                {
                    ...
                    _logger.Info($"Listener '{nameof(TemplateEventListener2)}' - response from '{resourceUri}' - '{t.Result}' - '{DateTime.UtcNow.ToString("yyyy/MM/dd hh:mm:ss")}'");
